...
if [ "$tlversion" = "$default_version" ]; then
    ...
    # in sync.
    installer_url=$(curl -L -v -o /dev/null -w '%{url_effective}' --dns-servers '1.1.1.1' http://mirror.ctan.org/systems/texlive/tlnet)
    repository=
else
    installer_url="\
...
curl -v -o "$installer_archive" --dns-servers '1.1.1.1' "$installer_url"/"$installer_archive"
curl -v -o "$installer_archive".sha512 --dns-servers '1.1.1.1' "$installer_url"/"$installer_archive".sha512
curl -v -o "$installer_archive".sha512.asc --dns-servers '1.1.1.1' "$installer_url"/"$installer_archive".sha512.asc
